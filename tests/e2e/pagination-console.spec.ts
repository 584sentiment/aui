...
test.describe('Pagination 组件浏览器控制台测试', () => {
  test('Pagination 页面不应有控制台错误', async ({ page }) => {
    // 在页面中注入脚本来捕获所有控制台消息
    ...
    // 访问 Pagination 文档页面
    await page.goto('http://localhost:5175/components/Pagination.html')
    ...
    const vueErrors = consoleMessages.filter((m: any) =>
      m.args.some((a: string) =>
        a.includes('[Vue warn]') ||
        a.includes('Uncaught') ||
        a.includes('does not provide an export') ||
        a.includes('Missing required prop')
      )
    )
    ...
    // 截图保存
    await page.screenshot({ path: 'test-results/pagination-screenshot.png', fullPage: true })
    console.log('\n截图已保存到: test-results/pagination-screenshot.png')
  ...
  test('Pagination 交互测试', async ({ page }) => {
    const errors: string[] = []
    ...
    // 查找并点击分页按钮
    const pageButtons = page.locator('.pagination-wrapper button').all()
    const count = await pageButtons.length

    console.log(`找到 ${count} 个分页按钮`)

    // 点击前几个按钮
    for (let i = 0; i < Math.min(count, 3); i++) {
      const button = page.locator('.pagination-wrapper button').nth(i)
